(** The topos gate for the store_notify_visibility family:
    {!Store_notify_visibility_model} runs on [Denote.Make], so it owes the three
...
module G =
  Topos_gate.Make
    (Store_notify_visibility_model.State)
    (Store_notify_visibility_model.View)

(** Pristine plus every gate deletion this family pins a statement with. *)
let muts =
  [
    Store_notify_visibility_model.Pristine;
    Store_notify_visibility_model.No_post_registration_reread;
    Store_notify_visibility_model.No_notify_fanout;
    Store_notify_visibility_model.No_mem_write_on_insert;
  ]

(** Case labels; exhaustive over the mutation sum. *)
let mut_name = function
  | Store_notify_visibility_model.Pristine -> "pristine"
  | Store_notify_visibility_model.No_post_registration_reread ->
      "no-post-registration-reread"
  | Store_notify_visibility_model.No_notify_fanout -> "no-notify-fanout"
  | Store_notify_visibility_model.No_mem_write_on_insert ->
      "no-mem-write-on-insert"

(** Every subformula of every statement, formula and antecedent alike. *)
let statement_formulas =
  List.concat_map
    (fun st ->
      Topos_gate.subformulas st.Store_notify_visibility_statements.formula
      @ Topos_gate.subformulas st.Store_notify_visibility_statements.antecedent)
...
(** Run the gate under one mutation, or fail on an impossible empty init. The
    init list is {!Store_notify_visibility_model.inits}, exactly as
    [spec_of] uses it: the honest-parent branch and the fabricated-parent
    branch. *)
let with_verdict mut k =
  Result.fold
    ~error:(fun G.Empty_init -> Alcotest.fail "topos gate: empty init")
    ~ok:k
    (G.run ~init:Store_notify_visibility_model.inits
       ~next:(Store_notify_visibility_model.next_with mut)
       ~view:Store_notify_visibility_model.view
       ~label:Store_notify_visibility_model.label ~formulas)

(** [W] is a genuine finite poset: reflexive, transitive and antisymmetric.
    Every transition of this family strictly advances a monotone component
    ([written], [mem], [dur], [deliv_closed], or a handler along
    [W_idle < W_want < W_reg < W_done/W_lost]) and none moves one back, so the
    reachability relation is acyclic. *)
...
let reflection_non_vacuous () =
  with_verdict Store_notify_visibility_model.Pristine (fun v ->
      Alcotest.(check bool)
        "deleting the classical bridge flips a verdict on this family" true
        v.G.reflection_load_bearing)

let () =
  Alcotest.run "store_notify_visibility-topos"
    [
      ( "poset",
        List.map
