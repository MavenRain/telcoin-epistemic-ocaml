(** The topos gate for the store_full_memory family:
    {!Store_full_memory_model} runs on [Denote.Make], so it owes the three
...
module G =
  Topos_gate.Make (Store_full_memory_model.State) (Store_full_memory_model.View)

(** Pristine plus every gate deletion this family pins a statement with. *)
let muts =
  [
    Store_full_memory_model.Pristine;
    Store_full_memory_model.No_open_preload;
    Store_full_memory_model.No_full_memory_mode_test;
    Store_full_memory_model.No_mem_chain_in_iter;
  ]

(** Case labels; exhaustive over the mutation sum. *)
let mut_name = function
  | Store_full_memory_model.Pristine -> "pristine"
  | Store_full_memory_model.No_open_preload -> "no-open-preload"
  | Store_full_memory_model.No_full_memory_mode_test ->
      "no-full-memory-mode-test"
  | Store_full_memory_model.No_mem_chain_in_iter -> "no-mem-chain-in-iter"

(** Every subformula of every statement, formula and antecedent alike. *)
let statement_formulas =
  List.concat_map
    (fun st ->
      Topos_gate.subformulas st.Store_full_memory_statements.formula
      @ Topos_gate.subformulas st.Store_full_memory_statements.antecedent)
...
let with_verdict mut k =
  Result.fold
    ...
    ~ok:k
    (G.run
       ~init:[ Store_full_memory_model.initial ]
       ~next:(Store_full_memory_model.next_with mut)
       ~view:Store_full_memory_model.view ~label:Store_full_memory_model.label
       ~formulas)
