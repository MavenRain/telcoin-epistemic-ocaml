(** The topos gate for the batch_quorum_tally family:
    {!Batch_quorum_tally_model} runs on [Denote.Make], so it owes the three
...
module G =
  Topos_gate.Make (Batch_quorum_tally_model.State) (Batch_quorum_tally_model.View)

(** Pristine plus every gate deletion this family pins a statement with. *)
let muts =
  [
    Batch_quorum_tally_model.Pristine;
    Batch_quorum_tally_model.No_author_self_credit;
    Batch_quorum_tally_model.No_recoverable_class;
    Batch_quorum_tally_model.Truncated_fanout;
  ]

(** Case labels; exhaustive over the mutation sum. *)
let mut_name = function
  | Batch_quorum_tally_model.Pristine -> "pristine"
  | Batch_quorum_tally_model.No_author_self_credit -> "no-author-self-credit"
  | Batch_quorum_tally_model.No_recoverable_class -> "no-recoverable-class"
  | Batch_quorum_tally_model.Truncated_fanout -> "truncated-fanout"

(** Every subformula of every statement, formula and antecedent alike. *)
let statement_formulas =
  List.concat_map
    (fun st ->
      Topos_gate.subformulas st.Batch_quorum_tally_statements.formula
      @ Topos_gate.subformulas st.Batch_quorum_tally_statements.antecedent)
...
let with_verdict mut k =
  Result.fold
    ...
    ~ok:k
    (G.run
       ~init:[ Batch_quorum_tally_model.initial ]
       ~next:(Batch_quorum_tally_model.next_with mut)
       ~view:Batch_quorum_tally_model.view ~label:Batch_quorum_tally_model.label
       ~formulas)

(** [W] is a genuine finite poset: reflexive, transitive and antisymmetric.
    Every edge either dispatches one more peer or consumes one more response, so
    [offered + resolved] strictly increases and no state is ever returned to. *)
