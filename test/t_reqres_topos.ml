(** The topos gate for the reqres family: {!Reqres_model} runs on
...
module G = Topos_gate.Make (Reqres_model.State) (Reqres_model.View)

(** Pristine plus every gate deletion this family pins a statement with. *)
let muts =
  [
    Reqres_model.Pristine;
    Reqres_model.No_digest_gate;
  ]

(** Case labels; exhaustive over the mutation sum. *)
let mut_name = function
  | Reqres_model.Pristine -> "pristine"
  | Reqres_model.No_digest_gate -> "no-digest-gate"

(** Every subformula of every statement, formula and antecedent alike. *)
let statement_formulas =
  List.concat_map
    (fun st ->
      Topos_gate.subformulas st.Reqres_statements.formula
      @ Topos_gate.subformulas st.Reqres_statements.antecedent)
...
let with_verdict mut k =
  Result.fold
    ...
    ~ok:k
    (G.run ~init:[ Reqres_model.initial ] ~next:(Reqres_model.next_with mut)
       ~view:Reqres_model.view ~label:Reqres_model.label ~formulas)
