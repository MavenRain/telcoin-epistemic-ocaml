(** The topos gate for the epoch_close family: {!Epoch_close_model} runs on
...
module G = Topos_gate.Make (Epoch_close_model.State) (Epoch_close_model.View)

(** Pristine plus every gate deletion this family pins a statement with. *)
let muts =
  [
    Epoch_close_model.Pristine;
    Epoch_close_model.No_cert_quorum_count;
    Epoch_close_model.No_cert_digest_binding;
    Epoch_close_model.No_committee_predial;
  ]

(** Case labels; exhaustive over the mutation sum. *)
let mut_name = function
  | Epoch_close_model.Pristine -> "pristine"
  | Epoch_close_model.No_cert_quorum_count -> "no-cert-quorum-count"
  | Epoch_close_model.No_cert_digest_binding -> "no-cert-digest-binding"
  | Epoch_close_model.No_committee_predial -> "no-committee-predial"

(** Every subformula of every statement, formula and antecedent alike. *)
let statement_formulas =
  List.concat_map
    (fun st ->
      Topos_gate.subformulas st.Epoch_close_statements.formula
      @ Topos_gate.subformulas st.Epoch_close_statements.antecedent)
...
let with_verdict mut k =
  Result.fold
    ...
    ~ok:k
    (G.run ~init:[ Epoch_close_model.initial ] ~next:(Epoch_close_model.next_with mut)
       ~view:Epoch_close_model.view ~label:Epoch_close_model.label ~formulas)
