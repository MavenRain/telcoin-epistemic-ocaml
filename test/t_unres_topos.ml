(** The topos gate for the unres family: {!Unres_model} runs on
...
module G = Topos_gate.Make (Unres_model.State) (Unres_model.View)

(** Pristine plus every gate deletion this family pins a statement with. *)
let muts =
  [
    Unres_model.Pristine;
    Unres_model.No_author_resolved_guard;
  ]

(** Case labels; exhaustive over the mutation sum. *)
let mut_name = function
  | Unres_model.Pristine -> "pristine"
  | Unres_model.No_author_resolved_guard -> "no-author-resolved-guard"

(** Every subformula of every statement, formula and antecedent alike. *)
let statement_formulas =
  List.concat_map
    (fun st ->
      Topos_gate.subformulas st.Unres_statements.formula
      @ Topos_gate.subformulas st.Unres_statements.antecedent)
...
let with_verdict mut k =
  Result.fold
    ...
    ~ok:k
    (G.run ~init:[ Unres_model.initial ] ~next:(Unres_model.next_with mut)
       ~view:Unres_model.view ~label:Unres_model.label ~formulas)
