(** The topos gate for the swap family: {!Swap_model} runs on
...
module G = Topos_gate.Make (Swap_model.State) (Swap_model.View)

(** Pristine plus every gate deletion this family pins a statement with. *)
let muts =
  [
    Swap_model.Pristine;
    Swap_model.No_shared_seed;
  ]

(** Case labels; exhaustive over the mutation sum. *)
let mut_name = function
  | Swap_model.Pristine -> "pristine"
  | Swap_model.No_shared_seed -> "no-shared-seed"

(** Every subformula of every statement, formula and antecedent alike. *)
let statement_formulas =
  List.concat_map
    (fun st ->
      Topos_gate.subformulas st.Swap_statements.formula
      @ Topos_gate.subformulas st.Swap_statements.antecedent)
    Swap_statements.all
...
let with_verdict mut k =
  Result.fold
    ...
    ~ok:k
    (G.run ~init:[ Swap_model.initial ] ~next:(Swap_model.next_with mut)
       ~view:Swap_model.view ~label:Swap_model.label ~formulas)
