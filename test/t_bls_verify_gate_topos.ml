(** The topos gate for the bls_verify_gate family: {!Bls_verify_gate_model} runs
    ...
    - [W] is a genuine finite poset, so the presheaf topos over it is the
      ...
      because the call life [No_call -> Pending -> Settled] is a three-layer DAG
      and nothing undoes itself, so reachability is antisymmetric;
...
module G =
  Topos_gate.Make (Bls_verify_gate_model.State) (Bls_verify_gate_model.View)

(** Pristine plus every gate deletion this family pins a statement with. *)
let muts =
  [
    Bls_verify_gate_model.Pristine;
    Bls_verify_gate_model.Err_on_failed_verify;
    Bls_verify_gate_model.No_length_gate;
    Bls_verify_gate_model.No_pk_validation;
  ]

(** Case labels; exhaustive over the mutation sum. *)
let mut_name = function
  | Bls_verify_gate_model.Pristine -> "pristine"
  | Bls_verify_gate_model.Err_on_failed_verify -> "err-on-failed-verify"
  | Bls_verify_gate_model.No_length_gate -> "no-length-gate"
  | Bls_verify_gate_model.No_pk_validation -> "no-pk-validation"

(** Every subformula of every statement, formula and antecedent alike. *)
let statement_formulas =
  List.concat_map
    (fun st ->
      Topos_gate.subformulas st.Bls_verify_gate_statements.formula
      @ Topos_gate.subformulas st.Bls_verify_gate_statements.antecedent)
...
let with_verdict mut k =
  Result.fold
    ...
    ~ok:k
    (G.run
       ~init:[ Bls_verify_gate_model.initial ]
       ~next:(Bls_verify_gate_model.next_with mut)
       ~view:Bls_verify_gate_model.view ~label:Bls_verify_gate_model.label
       ~formulas)
