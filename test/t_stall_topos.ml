(** The topos gate for the stall family: {!Stall_model} runs on
...
module G = Topos_gate.Make (Stall_model.State) (Stall_model.View)

(** Pristine plus every gate deletion this family pins a statement with. *)
let muts =
  [
    Stall_model.Pristine;
    Stall_model.No_timeout_fetch;
  ]

(** Case labels; exhaustive over the mutation sum. *)
let mut_name = function
  | Stall_model.Pristine -> "pristine"
  | Stall_model.No_timeout_fetch -> "no-timeout-fetch"

(** Every subformula of every statement, formula and antecedent alike. *)
let statement_formulas =
  List.concat_map
    (fun st ->
      Topos_gate.subformulas st.Stall_statements.formula
      @ Topos_gate.subformulas st.Stall_statements.antecedent)
...
let with_verdict mut k =
  Result.fold
    ...
    ~ok:k
    (G.run ~init:[ Stall_model.initial ] ~next:(Stall_model.next_with mut)
       ~view:Stall_model.view ~label:Stall_model.label ~formulas)
