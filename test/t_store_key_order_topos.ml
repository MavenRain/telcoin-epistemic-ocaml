(** The topos gate for the store_key_order family: {!Store_key_order_model}
    ...
    - [W] is a genuine finite poset: the index only ever gains rows, an
      authority's signature is only ever set, and the fetch decision only ever
      advances from [Filling] to [Probed] to [Decided], so no state is ever
      returned to and {!Frame.certify_functorial} gets antisymmetry;
...
module G =
  Topos_gate.Make (Store_key_order_model.State) (Store_key_order_model.View)

(** Pristine plus every gate deletion this family pins a statement with. *)
let muts =
  [
    Store_key_order_model.Pristine;
    Store_key_order_model.No_big_endian;
    Store_key_order_model.No_origin_guard;
  ]

(** Case labels; exhaustive over the mutation sum. *)
let mut_name = function
  | Store_key_order_model.Pristine -> "pristine"
  | Store_key_order_model.No_big_endian -> "no-big-endian"
  | Store_key_order_model.No_origin_guard -> "no-origin-guard"

(** Every subformula of every statement, formula and antecedent alike. *)
let statement_formulas =
  List.concat_map
    (fun st ->
      Topos_gate.subformulas st.Store_key_order_statements.formula
      @ Topos_gate.subformulas st.Store_key_order_statements.antecedent)
...
(** Run the gate under one mutation, or fail on an impossible empty init. The
    init list matches [spec_of] exactly: the single empty-index state. *)
let with_verdict mut k =
  Result.fold
    ~error:(fun G.Empty_init -> Alcotest.fail "topos gate: empty init")
    ~ok:k
    (G.run
       ~init:[ Store_key_order_model.initial ]
       ~next:(Store_key_order_model.next_with mut)
       ~view:Store_key_order_model.view ~label:Store_key_order_model.label
       ~formulas)
