(** The topos gate for the admission family: {!Admission_model} runs on
...
module G = Topos_gate.Make (Admission_model.State) (Admission_model.View)

(** Pristine plus every gate deletion this family pins a statement with. *)
let muts =
  [
    Admission_model.Pristine;
    Admission_model.No_admission_denial;
  ]

(** Case labels; exhaustive over the mutation sum. *)
let mut_name = function
  | Admission_model.Pristine -> "pristine"
  | Admission_model.No_admission_denial -> "no-admission-denial"

(** Every subformula of every statement, formula and antecedent alike. *)
let statement_formulas =
  List.concat_map
    (fun st ->
      Topos_gate.subformulas st.Admission_statements.formula
      @ Topos_gate.subformulas st.Admission_statements.antecedent)
...
let with_verdict mut k =
  Result.fold
    ...
    ~ok:k
    (G.run ~init:[ Admission_model.initial ] ~next:(Admission_model.next_with mut)
       ~view:Admission_model.view ~label:Admission_model.label ~formulas)
