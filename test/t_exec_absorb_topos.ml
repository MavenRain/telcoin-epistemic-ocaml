(** The topos gate for the EXEC_ABSORB family: {!Exec_absorb_model} runs on
...
module G =
  Topos_gate.Make (Exec_absorb_model.State) (Exec_absorb_model.View)

(** Pristine plus every gate deletion this family pins a statement with. *)
let muts =
  [
    Exec_absorb_model.Pristine;
    Exec_absorb_model.Fatal_on_invalid_tx;
    Exec_absorb_model.No_validation_decode;
    Exec_absorb_model.Commit_partial_block;
  ]

(** Case labels; exhaustive over the mutation sum. *)
let mut_name = function
  | Exec_absorb_model.Pristine -> "pristine"
  | Exec_absorb_model.Fatal_on_invalid_tx -> "fatal-on-invalid-tx"
  | Exec_absorb_model.No_validation_decode -> "no-validation-decode"
  | Exec_absorb_model.Commit_partial_block -> "commit-partial-block"

(** Every subformula of every statement, formula and antecedent alike. *)
let statement_formulas =
  List.concat_map
    (fun st ->
      Topos_gate.subformulas st.Exec_absorb_statements.formula
      @ Topos_gate.subformulas st.Exec_absorb_statements.antecedent)
    Exec_absorb_statements.all

(** The checked set: the statements plus the spanning battery, seeded on the
    family's own atoms and read at the two executing nodes, which are the only
    agents with a non-constant view. *)
let formulas =
  statement_formulas
  @ Topos_gate.battery
      ~atoms:(Topos_gate.seeds statement_formulas)
      ~agent:Validator.V0
      ~group:[ Validator.V0; Validator.V1 ]

(** Run the gate under one mutation, or fail on an impossible empty init. The
    [init] list mirrors {!Exec_absorb_model.spec_of} exactly. *)
let with_verdict mut k =
  Result.fold
    ~error:(fun G.Empty_init -> Alcotest.fail "topos gate: empty init")
    ~ok:k
    (G.run ~init:Exec_absorb_model.inits
       ~next:(Exec_absorb_model.next_with mut) ~view:Exec_absorb_model.view
       ~label:Exec_absorb_model.label ~formulas)
