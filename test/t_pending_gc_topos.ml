(** The topos gate for the pending_gc family: {!Pending_gc_model} runs on
...
module G = Topos_gate.Make (Pending_gc_model.State) (Pending_gc_model.View)

(** Pristine plus every gate deletion this family pins a statement with. *)
let muts =
  [
    Pending_gc_model.Pristine;
    Pending_gc_model.No_gc_release;
    Pending_gc_model.No_parent_check;
    Pending_gc_model.No_pending_filter;
    Pending_gc_model.No_parent_wait;
  ]

(** Case labels; exhaustive over the mutation sum. *)
let mut_name = function
  | Pending_gc_model.Pristine -> "pristine"
  | Pending_gc_model.No_gc_release -> "no-gc-release"
  | Pending_gc_model.No_parent_check -> "no-parent-check"
  | Pending_gc_model.No_pending_filter -> "no-pending-filter"
  | Pending_gc_model.No_parent_wait -> "no-parent-wait"

(** Every subformula of every statement, formula and antecedent alike. *)
let statement_formulas =
  List.concat_map
    (fun st ->
      Topos_gate.subformulas st.Pending_gc_statements.formula
      @ Topos_gate.subformulas st.Pending_gc_statements.antecedent)
...
let with_verdict mut k =
  Result.fold
    ...
    ~ok:k
    (G.run ~init:[ Pending_gc_model.initial; Pending_gc_model.initial_peer_holds ] ~next:(Pending_gc_model.next_with mut)
       ~view:Pending_gc_model.view ~label:Pending_gc_model.label ~formulas)
