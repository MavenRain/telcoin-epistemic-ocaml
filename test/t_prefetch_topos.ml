(** The topos gate for the prefetch family: {!Prefetch_model} runs on
...
module G = Topos_gate.Make (Prefetch_model.State) (Prefetch_model.View)

(** Pristine plus every gate deletion this family pins a statement with. *)
let muts =
  [
    Prefetch_model.Pristine;
    Prefetch_model.No_content_addressing;
  ]

(** Case labels; exhaustive over the mutation sum. *)
let mut_name = function
  | Prefetch_model.Pristine -> "pristine"
  | Prefetch_model.No_content_addressing -> "no-content-addressing"

(** Every subformula of every statement, formula and antecedent alike. *)
let statement_formulas =
  List.concat_map
    (fun st ->
      Topos_gate.subformulas st.Prefetch_statements.formula
      @ Topos_gate.subformulas st.Prefetch_statements.antecedent)
...
let with_verdict mut k =
  Result.fold
    ...
    ~ok:k
    (G.run ~init:[ Prefetch_model.initial ] ~next:(Prefetch_model.next_with mut)
       ~view:Prefetch_model.view ~label:Prefetch_model.label ~formulas)
