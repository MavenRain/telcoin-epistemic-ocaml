(** The topos gate for the backend_env_split family:
    {!Backend_env_split_model} runs on [Denote.Make], so it owes the three
...
module G =
  Topos_gate.Make (Backend_env_split_model.State) (Backend_env_split_model.View)

(** Pristine plus every gate deletion this family pins a statement with. *)
let muts =
  [
    Backend_env_split_model.Pristine;
    Backend_env_split_model.Merge_cache_into_epoch_env;
    Backend_env_split_model.No_redb_table_recreate;
  ]

(** Case labels; exhaustive over the mutation sum. *)
let mut_name = function
  | Backend_env_split_model.Pristine -> "pristine"
  | Backend_env_split_model.Merge_cache_into_epoch_env ->
      "merge-cache-into-epoch-env"
  | Backend_env_split_model.No_redb_table_recreate -> "no-redb-table-recreate"

(** Every subformula of every statement, formula and antecedent alike. *)
let statement_formulas =
  List.concat_map
    (fun st ->
      Topos_gate.subformulas st.Backend_env_split_statements.formula
      @ Topos_gate.subformulas st.Backend_env_split_statements.antecedent)
...
let with_verdict mut k =
  Result.fold
    ...
    ~ok:k
    (G.run
       ~init:[ Backend_env_split_model.initial ]
       ~next:(Backend_env_split_model.next_with mut)
       ~view:Backend_env_split_model.view ~label:Backend_env_split_model.label
       ~formulas)
