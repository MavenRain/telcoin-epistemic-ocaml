(** The topos gate for the revote family: {!Revote_model} runs on
...
module G = Topos_gate.Make (Revote_model.State) (Revote_model.View)

(** Pristine plus every gate deletion this family pins a statement with. *)
let muts =
  [
    Revote_model.Pristine;
    Revote_model.No_cert_evidence_guard;
  ]

(** Case labels; exhaustive over the mutation sum. *)
let mut_name = function
  | Revote_model.Pristine -> "pristine"
  | Revote_model.No_cert_evidence_guard -> "no-cert-evidence-guard"

(** Every subformula of every statement, formula and antecedent alike. *)
let statement_formulas =
  List.concat_map
    (fun st ->
      Topos_gate.subformulas st.Revote_statements.formula
      @ Topos_gate.subformulas st.Revote_statements.antecedent)
...
let with_verdict mut k =
  Result.fold
    ...
    ~ok:k
    (G.run ~init:[ Revote_model.initial ] ~next:(Revote_model.next_with mut)
       ~view:Revote_model.view ~label:Revote_model.label ~formulas)
